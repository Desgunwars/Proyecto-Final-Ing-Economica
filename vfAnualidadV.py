...
class VfAnualidadV(object):
    def __init__(self):
        super().__init__()
        self.newWindow = tk.Toplevel()
        self.newWindow.title = "Valor Final Anualidad Vencida"
        self.newWindow.geometry('500x230')
        self.newWindow.resizable(0, 0)        

        self.wrapper = tk.LabelFrame(self.newWindow, text="Valor Final Anualidad Vencida")
        ...
        self.mensajeLabelRuslt = tk.Label(self.wrapper, text="La Anualidad Vencida del valor final es: ")
        self.mensajeReult = tk.Label(self.wrapper, textvariable = self.vf, font=("Time New Roman", 10, "bold"))
        
        # Buttons
        self.calcularValorFInal = tk.Button(self.wrapper, text="Calcular Anualidad Vencida", width=25, command=self.CalcularVfAnualidadV)
    ...
    def CalcularVfAnualidadV(self):
        if self.A.get() == "" or self.i.get() == "" or self.n.get() == "" :
            ...
            multi = (cuota * (((1 + tasaEfectivaAnual)**nperiodos) - 1)/tasaEfectivaAnual) 
